...
let proximoOrcamentoId = parseInt(localStorage.getItem('proximoOrcamentoId')) || 1;

let currentOrcamento = JSON.parse(localStorage.getItem('currentOrcamento')) || {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // A renderização de notas foi removida daqui pois agora é integrada na aba
...
function autoSaveHandler(callback, event) {
    if (event.type === 'submit') { // Salva no submit para garantir
        event.preventDefault();
        callback(event, true); // Passa um flag para indicar que é um salvamento final
    } else { // Salva no input (automático)
        callback(event, false); // Flag indica que é salvamento automático
    }
}
...
function generateSequentialId(isInitialLoad = false) {
    ...
    if (!isInitialLoad) {
        proximoOrcamentoId++;
        localStorage.setItem('proximoOrcamentoId', proximoOrcamentoId.toString());
    }
...
function carregarOrcamentoAtual() {
    currentOrcamento = JSON.parse(localStorage.getItem('currentOrcamento')) || { id: generateSequentialId(true), clienteId: null, itens: [], maoDeObra: 0, relatorio: "", formasPagamento: "", servicos: "", faturamentoData: null, nfeChaves: [] };
    if (!currentOrcamento.id) currentOrcamento.id = generateSequentialId(true);
    // Assegurar que nfeChaves é um array e, se for de strings antigas, converter para objetos
    if (!Array.isArray(currentOrcamento.nfeChaves)) {
        currentOrcamento.nfeChaves = [];
    } else {
        // Converte chaves antigas (strings) para o novo formato de objeto
        currentOrcamento.nfeChaves = currentOrcamento.nfeChaves.map(entry => {
            if (typeof entry === 'string') {
                return { chave: entry, fornecedor: '' };
            }
            return entry; // Já é um objeto, ou null/undefined que manterá como está
        }).filter(entry => entry !== null && entry !== undefined); // Remove entradas nulas/indefinidas
    }
...
function renderizarOrcamentosSalvos() {
    ...
    orcamentos.sort((a, b) => new Date(b.data) - new Date(a.data)).forEach(orc => {
        ...
        const hasNFE = orc.nfeChaves && orc.nfeChaves.length > 0;
        ul.innerHTML += `
            <li>
